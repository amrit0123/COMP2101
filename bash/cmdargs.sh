...
#using case statement to recognize options
echo "Processing the value '$1'."
case $1 in
-h )
#printing help command options
  echo 'adding "-h" command for help.'
  echo "Choose -v for Varbose, -d for Debug and debug level within 1 to 5."
exit
  ;;
-v )
#prrinting verbose command option
  echo 'adding "-v" command for varbose.'
  verboseMode=1
  ;;
-d )
#printing debug command options
  echo 'adding "-d" command for debug.'
  case $2 in
  [1-5] )
...
#running task 2, displaying setting of verbose and debug
