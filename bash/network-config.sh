...
#giving variables to the outputs
myHostname=$(hostname)
interfaceName=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lanAddress=$(ip a s $interfaceName|awk '/inet /{gsub(/\/.*/,"");print $2}')
lanHostname=$(getent hosts $(ip a s $(interfaceName)|awk '/inet /{gsub(/\/.*/,"");print $2}')| awk '{print $2}')
externalIP=$(curl -s icanhazip.com)
externalName=$(getent hosts $externalIP| awk '{print $2}')
RouterAddress= $(ip r s default| cut -d ' ' -f 3)
RouterHostname=$(getnet hosts $RouterAddress|awk '{print $2}')


#showing the results that would be visible by using variable names
cat <<EOF
Hostname        : myHostname
